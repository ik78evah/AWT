...
class LoginView extends Component {
    ...
    async signIn(username, password) {
        const response = await fetch("/login", {
            method: "POST",
            headers: {'Content-Type': 'application/json', 'ProjectId': 1},
            body: JSON.stringify({
                username: username,
                password: password,
            })
        ...
        return (
            <div className="h-full w-full flex items-center justify-center py-16 px-4 sm:px-6 lg:px-8">
                <div className="max-w-md w-full space-y-8">
                    <div>
                        <h1 className="text-center text-3xl font-bold font-mono text-medium block py-2 pl-3 pr-4 text-red-500 border-b border-gray-100 md:border-0 md:p-0">
                            🗓 ToDo List
                        </h1>
                        ...
                        <div className={"space-y-3"}>
                            <button id="actionButton" className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-red-500 hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                {this.state.isRegisterMode ? "Register" : "Sign In"}
                            </button>
                            <button id="alternativeButton" className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-red-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
